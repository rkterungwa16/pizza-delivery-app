...
class DataStorageUtils {
  ...
  /**
   * Implement extract contents of all files in a directory
   * @param {String} dirName
   */
  _readDirectoryFilesContent (dirName) {
    // let contentOfDirectoryFiles
    return new Promise((resolve, reject) => {
      this._readDirectory(`${this.baseDir}${dirName}`)
        .then((fileNames) => {
          const contentOfDirectoryFiles = fileNames.map((fileName) => {
            return this._readFile(dirName, `${fileName}`)
          })
          return Promise.all(contentOfDirectoryFiles)
        })
